...
export default function Chatbot() {
  ...
  // (e.g. "you haven't uploaded anything yet"). Silent fail is fine — it's
  // just used to enrich replies.
  useEffect(() => {
    fileAPI
      .listUploads()
  ...
  useEffect(() => {
    if (isOpen) {
      // Wait one tick so the panel mounts first.
  ...
  const sendMessage = (rawText) => {
    ...
    // 2) Simulate "thinking" delay — 900–1400ms so it feels like an LLM
    //    without being annoyingly slow.
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className={styles.panel} role="dialog" aria-label="PM OS Assistant">
